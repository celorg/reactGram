...
const insertPhoto = async(req,res) => {
    ...
    const user = await User.findOne({_id: reqUser._id}).select("-password");
...
const getUserPhotos = async(req,res) => {
    ...
    try{
        ...
        res.status(404).json({error: ["Usúario não encontrado!"]})
    }
    
}

const getAllPhostAndUser = async(req,res) => {
    
    const users = await User.find({}).select("-password");

    const photos = await Photo.find({});

    return res.status(200).json({users,photos})
...
const updatePhoto = async(req,res) => {
    ...
    //check if photo exists
    if(!photo){
        res.status(404).json({errors: ["Essa foto não encontrada"]});
        return;
    }
...
const searchPhotos = async(req,res) => {
    ...
    const photos = await Photo.find({titile: new RegExp(q, "i")}).exec();
